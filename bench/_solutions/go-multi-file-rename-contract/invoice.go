...
// computeLine prices one invoice line under the shop's discount rule, for
// the given customer tier.
func computeLine(d Discounter, item Item, tier Tier) InvoiceLine {
...
func GenerateInvoice(c Cart) Invoice {
	...
	for _, item := range c.Items {
		line := computeLine(c.Discount, item, c.Customer.Tier)
