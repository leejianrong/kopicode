...
// Isolate puts cmd in a process group of its own, so that everything it spawns
// can be signalled with it.
//
// Setpgid makes the child a group leader, so its pid doubles as the group id
// and its descendants inherit that group. That is what lets one signal reach
// the whole tree. Without it the tree has no name, and the only thing that can
// be killed is the process we happen to hold a handle on.
//
// An existing SysProcAttr is preserved rather than replaced: run_shell's
// Windows path sets CmdLine on the same struct, and a helper that quietly
// dropped a caller's other attributes would be a trap.
...
// Terminate asks the group to exit, and reports what was sent.
func Terminate(cmd *exec.Cmd) (string, error) {
	return signalGroup(cmd, syscall.SIGTERM)
}

// Kill ends the group whether it agrees or not.
...
//
// The negative pid is the mechanism this package rests on: kill(2) with a
// negative first argument signals every member of that process group, so a
// shell's backgrounded children and a compiler's subprocesses are reached along
// with their parent.
...
// ExitStatus reads a finished process's status: its exit code, and the name of
...
// these are the signals a subprocess plausibly dies of — and anything else
