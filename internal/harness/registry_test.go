...
//
// With one registered model, a confident wrong suggestion is worse than none:
// "did you mean qwen/qwen3-coder-next?" in answer to a request for an unrelated
// model reads as though the harness knows something it does not, while the
// supported list — always printed — already says everything true.
