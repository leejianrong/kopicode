...
change part of a file that exists, use `edit_file_fuzzy`: retyping a file is
how content gets lost.

### edit_file_fuzzy
`{"path": "...", "before": "...", "after": "..."}` — replaces text matched by
similarity, with whitespace normalised. `before` is text you believe is in the
file. It refuses when nothing is close enough, and refuses when more than one
region matches — give it enough surrounding context to identify one place
uniquely rather than a fragment that repeats.

A fuzzy edit that lands in the wrong place still reports success, which no
rejection can undo. Prefer a `before` long enough to be unambiguous over a
short one you will have to retry.
